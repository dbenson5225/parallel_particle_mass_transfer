...
np=20000; ens=1;  % # A points, #particles per point, # runs, point radius
...
Sx=[1]
Sx=[1 2 3 4 5 6 7 8 25];  %  # subdomains in one direction
Sx=1:25;
...
for n=1:ntsteps
  ...
  for i=1:ndim
    pad(i)=3*sigma; if(gpu>0); pad=gpuArray(pad); end;
...
%      zsmall=xlo(3,know);  zlarge=xhi(3,know);
      idxlocal=A(:,4)==ijk(1) & A(:,5)==ijk(2) & A(:,6)==ijk(3);

      idxghost=A(:,1)>lims(1,1) & A(:,1)<=lims(1,2) & ...
...
%      dM=beta*(P*X(:,7:end)-X(:,7:end));
      masstemp(idxghost,:)=P*X(:,7:end);              % keep new mass on all ghosts
